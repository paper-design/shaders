...
void main() {
    ...
    const int max_balls_number = 7;
    for (int i = 0; i < max_balls_number; i++) {
        vec2 pos = vec2(.5) + 1e-4;
        float angle = TWO_PI * float(i) / float(max_balls_number);
        pos += (2. - cos(.5 * t + angle) - sin(.4 * t - angle)) * vec2(sin(t + angle), cos(.5 * t - angle));
        ...
        shape *= smoothstep((float(i) - 1.) / float(max_balls_number), float(i) / float(max_balls_number), u_visibilityRange);
