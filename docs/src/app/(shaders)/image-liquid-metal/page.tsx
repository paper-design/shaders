...
const ImageLiquidMetalWithControls = () => {
  ...
  const [imageAlpha, setImageAlpha] = useState<HTMLImageElement | string>('/images/image-filters/paper.svg');
  ...
  const [params, setParams] = useControls(() => {
    ...
    return {
      frame: { value: defaults.frame, min: 0, max: 10000, order: 10 },
    ...
    <>
      <ShaderContainer shaderDef={imageLiquidMetalDef} currentParams={params}>
        <Suspense fallback={null}>
          <ImageLiquidMetal {...params} image={image} imageAlpha={imageAlpha} suspendWhenProcessingImage />
        </Suspense>
      </ShaderContainer>
