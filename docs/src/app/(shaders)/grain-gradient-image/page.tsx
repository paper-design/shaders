...
import { grainGradientImageMeta, GrainGradientImageShape, GrainGradientImageShapes, ShaderFit } from '@paper-design/shaders';
...
const GrainGradientImageWithControls = () => {
  ...
  const { colors, setColors } = useColors({
    ...
    maxColorCount: grainGradientImageMeta.maxColorCount,
  });

  const [params, setParams] = useControls(() => {
    return {
      colorBack: { value: toHsla(defaults.colorBack), order: 100 },
      blend: { value: defaults.blend, min: 0, max: 1, order: 101 },
      ...
      shape: {
        ...
        options: Object.keys(GrainGradientImageShapes) as GrainGradientImageShape[],
        order: 203,
      },
  ...
  useUrlParams(params, setParams, grainGradientImageDef, setColors);
  usePresetHighlight(grainGradientImagePresets, params);
  cleanUpLevaParams(params);

  return (
    <>
      <ShaderContainer shaderDef={grainGradientImageDef} currentParams={{ colors, ...params }}>
        <GrainGradientImage onClick={handleClick} {...params} colors={colors} image={image} />
      </ShaderContainer>
      <div onClick={handleClick} className="mx-auto mt-16 mb-48 w-fit text-base text-current/70 select-none">
        Click to change the sample image
      </div>
      <ShaderDetails shaderDef={grainGradientImageDef} currentParams={{ colors, ...params }} />
    </>
  );
};
