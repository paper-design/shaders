...
export function CopyButton({ text, className, icon = 'copy', label, copiedLabel, ...props }: CopyButtonProps) {
  ...
  return (
    ...
    >
      ...
      {label ? <span className="text-sm font-medium">{copied ? copiedLabel || 'Copied' : label}</span> : null}
    </button>
  );
}
